...
const UpdateTask = () => {
    ...
    // fetch data
    const { data: getTasks = [], isLoading: taskLoading } = useQuery({
        queryKey: ['getTasks',id],
        queryFn: async () => {
            const { data } = await axiosSecure(`/getTask/${id}`)
            return data
        }
    })
    ...
    const onSubmit = async (data) => {
        ...
        try {
            ...
            const { data } = await axiosSecure.put(`/updateTasks/${id}`, task)
            // console.log(addTest)

            if (data) {
                Swal.fire({
                    title: `Successfully Updated the ${title}!`,
                    ...
                    navigate('/dashboard/upcomingTasks')
                });
            } else {
    ...
    if (customLoader || taskLoading) {
        <Loader />
    }

    return (
        <div className='flex flex-col gap-7 justify-center items-center min-h-[calc(100vh-50px)] overflow-x-auto w-11/12 mx-auto rounded-lg glass shadow-2xl text-primary bg-cover bg-no-repeat bg-center relative'>
            <Helmet>
                <title>{import.meta.env.VITE_WEBSITE} | Update Task</title>
            </Helmet>
            ...
            {/* back to Upcoming task page */}
            <Link 
            to='/dashboard/upcomingTasks' 
                ...
                <form onSubmit={handleSubmit(onSubmit)} className="w-11/12 ">
                    ...
                    <div className='mt-4'>
                        ...
                        <textarea
                            ...
                            name='description'
                            defaultValue={getTasks.description}
                    ...
                    <div className='mt-4'>
                        ...
                        <input
                            ...
                            name='due_date'
                            defaultValue={getTasks.due_date}
                            // value={today}
                            className='block w-full px-4 py-2  border rounded-lg input input-bordered focus:border-blue-400 focus:ring-opacity-40  focus:outline-none focus:ring focus:ring-blue-300'
                            type='date'
                    ...
                    <div className='mt-4 form-control w-full'>
                        ...
                        <select
                            id='priority'
                            name='priority'
                            defaultValue={getTasks.priority}
                    ...
                    <div className='mt-4'>
                        ...
                        <input
                            ...
                            name='category'
                            defaultValue={getTasks.category}
                            className='block w-full px-4 py-2  border rounded-lg input input-bordered focus:border-blue-400 focus:ring-opacity-40  focus:outline-none focus:ring focus:ring-blue-300'
                            type='text'
